...
class WordTree:
	...
	def makeTree(self, wordFile):
		...
		print str(self.nodes)+" nodes created"
	...
	def exportWords(self):
		...
		while len(words):
			...
			for symbol in word[1:-1]:
				wordSymbolList.append(''.join(symbol))
			if wordSymbolList not in wordList:
				wordList.append(wordSymbolList)
		return wordList
...
def main(args):
	...
	if not prunedPicklePath:
		...
		if not fullPicklePath:
			with open(wordFilePath, 'r') as f:
				...
				if fullPicklePath == None:
					pickle.dump( wordTree, open( "pickles/tree.p", "wb" ))
		...
		if args.exportFullTree != False:
			if args.exportFullTree == None:
				fullTreeExportPath = 'exports/tree.json'
			else:
		...
		if prunedPicklePath == None:
			pickle.dump( wordTree, open( "pickles/tree_pruned.p", "wb" ))
	...
	if args.exportPrunedTree != False:
		if args.exportPrunedTree == None:
			prunedTreeExportPath = 'exports/pruned_tree.json'
		else:
	...
	if args.exportWords != False:
		if args.exportWords == None:
			wordExportFilePath = 'exports/treeWords.json'
		else:
			wordExportFilePath = args.exportWords
		with open(wordExportFilePath, 'wb') as f:
			wordList = wordTree.exportWords()
			json.dump(wordList, f)
		with open('exports/treeWords.txt', 'wb') as f:
...
if __name__ == "__main__":
	...
	parser.add_argument('--fullPickle', '-fp', default=False,  nargs='?', dest='fullPickle', help='Set flag with no arguments to write a pickle of the full tree to disk. Provide a single file path to read in a pickle from that destination.')
	parser.add_argument('--prunedPickle', '-pp', default=False,  nargs='?', dest='prunedPickle', help='Set flag with no arguments to write a pickle of the pruned tree to disk. Provide a single file path to read in a pickle from that destination.')
